...
public class SimulationSession {
    ...
    public LongSummaryStatistics runSimulation() {
        ...
        KelpPlant[][] kelpPlants = new KelpPlant[(int) Math.ceil(kelpCount / (double) SECTION_SIZE)][SECTION_SIZE];
        {
            int spawnedKelpPlants = 0;
            for (int group = 0; group < kelpPlants.length && spawnedKelpPlants < kelpCount; group++) {
                for (int i = 0; i < SECTION_SIZE && spawnedKelpPlants < kelpCount; i++) {
        ...
        for (long time = 0; time < testLength; time ++) {
            for (int i = 0; i < randomTickSpeed; i ++) {
                for (KelpPlant[] group : kelpPlants) {
                    while (true) {
                        final KelpPlant kelpPlant = group[random.nextInt(SECTION_SIZE)];
                        if (kelpPlant == null) continue;
                        kelpPlant.tick(random);
                        break;
                    }
                }
            }
